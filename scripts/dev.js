 ...
 * - 进程树清理仅 Windows 完备（taskkill /T /F）；POSIX 下 kill 只及直接子进程，
 *   本项目仅出 Windows 包（electron-builder --win），POSIX 分支是尽力而为。
...
const READY_TIMEOUT_MS = 120_000; // 端口解析成功后、HTTP 就绪的时限（无 MFSU 缓存的冷编译可能超过 1 分钟）
const POLL_INTERVAL_MS = 500;

/** 从 Umi stdout 的一行输出中解析 dev server 实际监听端口；无匹配返回 null */
function parseUmiPort(line) {
  const m = typeof line === 'string' ? line.match(PORT_MATCH) : null;
...
function main() {
  ...
  function exitAll(code) {
    ...
    electronPids.length = 0; // 已杀，清空以免 process.on('exit') 对同一 PID 二次 kill（PID 复用窗口期风险）
  ...
  pipeLines(umi, 'dev', (line) => {
    ...
    const parsed = parseUmiPort(line);
